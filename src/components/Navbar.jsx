    ...
    <nav className="lg:h-20 h-15 sticky  flex items-center justify-between rounded-full top-5 lg:ml-[115px] ml-5 backdrop-blur-md bg-blue-600/10  lg:w-[85%] w-[90%] text-white px-6 md:px-12 lg:px-20 z-20">
      ...
      <div>
        <ul className="hidden md:flex space-x-8">
          {links.map((i) =>(
            <li key={i} className="text-lg font-medium hover:underline" id={i} smooth><Link>{i}</Link></li>
          ))}
        </ul>
      </div>
      <button className="text-lg text-blue-600 bg-white px-5 py-2 font-medium rounded-full">Hire Me</button>
    </nav>
  );
};
